                         ...
                         GtkTextIter           *iter)
{
	...
	for (tags = files_view->priv->tags; tags != NULL; tags = g_slist_next (tags))
	{
		if (gtk_text_iter_has_tag (iter, tags->data))
		{
			tag = tags->data;
			break;
		}
...
/* for each line we get the sha and the real content.
 * for that sha we get the tag, and 2 things can happen:
 * - tag doesn't exists:
 *  - if a previous tag exits we apply the tag to the range
 *  - we create the tag for that sha
 *  - we get the revision for that sha and we set the revision to the tag
 * - tag exists:
 *  - it is the same as the previous line inserted, so the new text is inserted
 *  - it is different so we apply the tag to the range
 */
static void
                   ...
                   const gchar            *line)
{
	...
	if (tag == NULL || tag != tree->priv->active_tag)
	{
		if (tree->priv->active_tag != NULL)
		{
			...
			gtk_text_buffer_apply_tag (buffer, tree->priv->active_tag,
			                           &start, &iter);
			tree->priv->blame_offset = gtk_text_iter_get_offset (&iter);
		}
		...
		if (tag == NULL)
		{
			...
			g_hash_table_insert (tree->priv->blames, g_strdup (sha), tag);
			tree->priv->tags = g_slist_prepend (tree->priv->tags, tag);
		}
                 ...
                 GitgRevisionFilesView  *tree)
{
	...
	while ((line = *buffer++))
	{
		insert_blame_line (tree, buf, line);
	}
	...
	if (tree->priv->active_tag != NULL)
	{
		...
		gtk_text_buffer_get_iter_at_offset (buf, &start,
		                                    tree->priv->blame_offset);
		gtk_text_buffer_apply_tag (buf, tree->priv->active_tag,
		                           &start, &end);
